                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm">
                        <div class="card-body">
                            ...
                            <form method="POST" action="{{ route('sales.report.filter') }}">
                                @csrf
                                <div class="row">
                                    <!-- Date Range -->
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-12 mt-3">
                                        ...
                                        @if (count($sales) > 0)
                                        <button class="btn btn-light" onclick="window.open('{{ route('sales.report.print', ['date_range' => request('date_range')]) }}', '_blank')"><i class="bi bi-eye"></i>ពិនិត្យមើល</button>
                                        <a href="{{ route('sales.report.export', ['date_range' => request()->input('date_range')]) }}" class="btn btn-success">
                                            <i class="bi bi-download"></i> ទាញយកជា Excel
                                        </a>
                                            <a href="{{ route('sales.report.index') }}" class="btn btn-danger"> <i
                                            ...
                                            <tbody>
                                                @foreach ($sales as $sale)
                                                    <tr>
                                                        ...
                                                        <td>{{ \Carbon\Carbon::parse($sale->sale->date)->translatedFormat('d-F-Y') }}
                                                        </td> 
                                                        <td class="text-primary">{{ $sale->sale->reference }}</td>
                                                        <td>{{ $sale->sale->customer->name ?? 'N/A' }}</td>
                                                        <td>{{ $sale->quantity }}</td>
                                                        <td>${{ number_format($sale->unit_price, 2) }}</td>
                                                        <td>{{ $sale->discount }}</td>
                                                        <td>${{ number_format($sale->quantity * $sale->unit_price, 2) }}
                                                        </td>
