                    ...
                    <div class="col-sm-12">
                        <div class="page-sub-header">
                            <ul class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">របាយការណ៍ការបង្វិលចូល</li>
                            </ul>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body">
                            <h3 class="text-primary font-weight-600">
                                របាយការណ៍ការបង្វិលចូល
                            </h3>
                            <form method="POST" action="{{ route('sale-return.report.filter') }}">
                                @csrf
                                <div class="row">
                                    <!-- Date Range -->
                                    <div class="col-md-4">
                                    ...
                                    <!-- Customer -->
                                    <div class="col-md-4">
                                        <label for="customer_id">ជ្រើសរើសអតិថិជន</label>
                                    ...
                                    <div class="col-md-12 mt-3">
                                        ...
                                        @if (count($returns) > 0)
                                        <button class="btn btn-light" onclick="window.open('{{ route('sale-return.report.print', ['date_range' => request('date_range')]) }}', '_blank')"><i class="bi bi-eye"></i>ពិនិត្យមើល</button>
                                        <a href="{{ route('sale-return.export', ['date_range' => request()->input('date_range')]) }}" class="btn btn-success">
                                            <i class="bi bi-download"></i> ទាញយកជា Excel
                                        </a>
                                            <a href="{{ route('sale-return.report.index') }}" class="btn btn-danger"> <i
                                            ...
                                            <tbody>
                                                @foreach ($returns as $return)
                                                    <tr>
                                                        ...
                                                        <td>{{ \Carbon\Carbon::parse($return->saleReturn->date)->translatedFormat('d-F-Y') }}
                                                        </td> 
                                                        <td class="text-primary">{{ $return->saleReturn->reference }}</td>
                                                        <td>{{ $return->saleReturn->customer->name ?? 'N/A' }}</td>
                                                        <td>{{ $return->quantity }}</td>
                                                        <td>${{ number_format($return->unit_price, 2) }}</td>
                                                        <td>{{ $return->discount }}</td>
                                                        <td>${{ number_format($return->quantity * $return->unit_price, 2) }}
                                                        </td>
                                                        <td>{{ ucfirst($return->saleReturn->payment_method) }}</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
